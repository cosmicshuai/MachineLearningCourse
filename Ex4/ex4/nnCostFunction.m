...
Jcost=-yMat .* log(hx)- (1-yMat) .* log(1-hx);
...
J=J+(1/m)*sum(sum(Jcost));
J=J+(lambda/(2*m))*sum(sum(Theta1temp .* Theta1temp));
J=J+(lambda/(2*m))*sum(sum(Theta2temp .* Theta2temp));

diff3=hx-yMat;
z2=[ones(m,1),a1*Theta1'];
diff2=diff3*Theta2 .* sigmoidGradient(z2);
